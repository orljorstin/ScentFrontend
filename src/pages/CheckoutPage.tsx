...
// Fee: 100 PHP (~1.79 USD) for PH, 1500 PHP (~26.79 USD) for Intl
// We store/calc in USD, display in PHP
...
const handleSaveAddress = async () => {
    ...
    if (!formData.name || !formData.street || !formData.label || !formData.phone || !addrRegion || !addrCity || !addrBarangay) {
    ...
    const fullAddress = `${formData.street}, ${addrBarangay.name}, ${addrCity.name}, ${provinceStr}${addrRegion.name}, Philippines`;
...
const deliveryFee = 5.00; // Still static for now, task only mentioned implementing hook in this step
const total = cartTotal + deliveryFee;
                        ...
                        {showAddressForm && (
                            <div className="mb-6 p-4 border border-[#961E20]/20 rounded-xl bg-red-50/10 animate-fade-in">
                                ...
                                <div className="space-y-3">
                                    ...
                                    {/* Cascading Dropdowns */}
                                    <div className="grid grid-cols-2 gap-3">
                                        <select className="border p-2 rounded text-sm w-full" disabled>
